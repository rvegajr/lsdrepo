...
namespace LaserSportDataAPI.WebObjects
{
            ...
            if (InitConn(EventID, URL))
            {
                ...
                foreach (series _seriesItem in SeriesList.Values)
                {
                    ...
                    foreach (match _matchItem in SeriesMatchList)
                    {
                        ...
                        foreach (match_team_player_score score in MatchScoreList)
                        {
                            LSTeam team = new LSTeam(TeamList[score.team_id], score.score_override);
